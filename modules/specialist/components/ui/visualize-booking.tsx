...
interface InteractiveCalendarProps {
  ...
  onSaveLink?: (id: string, link: string) => Promise<void>;
  onConcludeSession?: (id: string) => Promise<void>;
}
...
const InteractiveCalendar: React.FC<InteractiveCalendarProps> = ({
  ...
  const handleSaveLink = async (appointmentId: string, link: string) => {
    ...
    if (onSaveLink) {
      await onSaveLink(appointmentId, link);
    } else {
      console.log('Saving link for appointment (mock):', appointmentId, link);
    }
  };

  const handleConcludeSessionWrapper = async (appointmentId: string) => {
    if (onConcludeSession) {
      await onConcludeSession(appointmentId);
    }
  };
                      ...
                      {day.appointments &&
                        day.appointments.map((appointment, idx) => (
                          <AppointmentCard
                            ...
                            handleConcludeSession={handleConcludeSessionWrapper}
...
interface AppointmentCardProps {
  ...
  handleConcludeSession?: (appointmentId: string) => void;
...
const AppointmentCard: React.FC<AppointmentCardProps> = ({
  ...
  const onConclude = async () => {
    if (confirm('Tem a certeza que deseja concluir esta sessão? Isso consumirá uma sessão do utilizador.')) {
      setCompleting(true);
      if (handleConcludeSession) {
        await handleConcludeSession(appointment.id);
      }
      setCompleting(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* Conclude Session Button - Only if confirmed/valid status */}
      {appointment.status !== 'completed' && appointment.status !== 'cancelled' && (
        <div className="mt-3">
          <Button
            onClick={onConclude}
            className="w-full text-sm bg-blue-600 hover:bg-blue-700 text-white font-inter font-semibold"
            disabled={completing}
          >
