...
interface DiagnosticFinanceiraProps {
  onBack: () => void;
  onComplete: () => void;
}

export function DiagnosticFinanceira({ onBack, onComplete }: DiagnosticFinanceiraProps) {
  ...
  const page1Question = "Como descreverias a tua situação financeira atual?";
  const page1Options = [
    "Estável — consigo pagar tudo e ainda poupar",
    "Controlada — mas sem margem para imprevistos",
    "Difícil — o dinheiro não chega até ao fim do mês",
    "Tensa — vivo com dívidas ou atrasos",
    "Evito pensar nisso, dá-me ansiedade"
  ];

  const page2Question = "O que mais se aplica a ti?";
  const page2Options = [
    "Tenho um plano financeiro claro",
    "Tento organizar-me, mas falho às vezes",
    "Gasto por impulso ou sem controlo",
    "Dependo de empréstimos ou ajuda frequente",
    "Não sei gerir o meu dinheiro e quero aprender"
  ];
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="text-center space-y-3 pt-4">
        <div className="flex items-center justify-center gap-3">
          ...
          <h1 className="font-poppins text-[#1565C0] text-[24px] font-semibold tracking-tight">Saúde Financeira</h1>
        </div>
        <div className="flex items-center justify-center gap-2">
          <div className={`w-2 h-2 rounded-full ${currentPage === 1 ? 'bg-[#388E3C]' : 'bg-gray-300'}`}></div>
          <div className={`w-2 h-2 rounded-full ${currentPage === 2 ? 'bg-[#388E3C]' : 'bg-gray-300'}`}></div>
        </div>
      </div>
