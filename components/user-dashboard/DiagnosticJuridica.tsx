...
interface DiagnosticJuridicaProps {
  onBack: () => void;
  onComplete: () => void;
}

export function DiagnosticJuridica({ onBack, onComplete }: DiagnosticJuridicaProps) {
  ...
  const page1Question = "Como te sentes em relação aos teus direitos e deveres legais?";
  const page1Options = [
    "Confiante — sei como agir em situações jurídicas",
    "Informado(a) — mas às vezes tenho dúvidas",
    "Confuso(a) — não sei bem o que é certo ou errado",
    "Inseguro(a) — evito lidar com assuntos legais",
    "Preocupado(a) — acho que posso ter problemas e não sei resolver"
  ];

  const page2Question = "Alguma destas situações já aconteceu contigo?";
  const page2Options = [
    "Dúvidas sobre contratos ou condições no trabalho",
    "Conflitos familiares (divórcio, pensão, guarda, etc.)",
    "Dívidas, multas ou processos legais",
    "Receio de injustiça no emprego ou comunidade",
    "Nenhuma, mas quero estar preparado(a)"
  ];
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="text-center space-y-3 pt-4">
        <div className="flex items-center justify-center gap-3">
          ...
          <h1 className="font-poppins text-[#1565C0] text-[24px] font-semibold tracking-tight">Saúde Jurídica</h1>
        </div>
        <div className="flex items-center justify-center gap-2">
          <div className={`w-2 h-2 rounded-full ${currentPage === 1 ? 'bg-[#10b981]' : 'bg-gray-300'}`}></div>
          <div className={`w-2 h-2 rounded-full ${currentPage === 2 ? 'bg-[#10b981]' : 'bg-gray-300'}`}></div>
        </div>
      </div>
