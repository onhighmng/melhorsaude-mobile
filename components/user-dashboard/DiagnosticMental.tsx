...
interface DiagnosticMentalProps {
  onBack: () => void;
  onComplete: () => void;
}

export function DiagnosticMental({ onBack, onComplete }: DiagnosticMentalProps) {
  ...
  const page1Question = "Como tens-te sentido nas últimas semanas?";
  const page1Options = [
    "Calmo(a) e equilibrado(a)",
    "Um pouco stressado(a), mas a lidar bem",
    "Cansado(a) e com dificuldade em relaxar",
    "Ansioso(a) ou sobrecarregado(a)",
    "Sem energia nem vontade de fazer nada"
  ];

  const page2Question = "O teu estado emocional tem afetado o teu trabalho ou vida pessoal?";
  const page2Options = [
    "Não, sinto-me bem e produtivo(a)",
    "Às vezes perco o foco ou paciência",
    "Tenho dificuldade em concentrar-me",
    "Tenho evitado responsabilidades ou pessoas",
    "Sinto que estou no limite e preciso de ajuda"
  ];
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="text-center space-y-3 pt-4">
        <div className="flex items-center justify-center gap-3">
          ...
          <h1 className="font-poppins text-[#1565C0] text-[24px] font-semibold tracking-tight">Saúde Mental</h1>
        </div>
        <div className="flex items-center justify-center gap-2">
          <div className={`w-2 h-2 rounded-full ${currentPage === 1 ? 'bg-[#1565C0]' : 'bg-gray-300'}`}></div>
          <div className={`w-2 h-2 rounded-full ${currentPage === 2 ? 'bg-[#1565C0]' : 'bg-gray-300'}`}></div>
        </div>
      </div>
          ...
          {currentOptions.map((option, index) => (
            <button
              ...
              className={`w-full p-4 rounded-full text-left transition-all flex items-center gap-3 shadow-sm ${
                currentAnswer === index
                  ? 'bg-white border-2 border-[#1565C0]'
