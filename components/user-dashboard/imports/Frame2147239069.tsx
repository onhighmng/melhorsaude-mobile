...
import imgImage from "@/assets/dashboard-logo-transparent.png";
import imgImageMelhorSaude from "@/assets/dashboard-logo-clean.png";
...
function Button4({ onClick }: { onClick?: () => void }) {
  return (
    <div
      className="relative rounded-[2.13569e+07px] shrink-0 size-[47.985px] cursor-pointer"
      data-name="Button"
      onClick={onClick}
      role="button"
      tabIndex={0}
...
function Button7({ onClick }: { onClick?: () => void }) {
  return (
    <div
      className="basis-0 bg-[#007aff] grow h-[47.985px] min-h-px min-w-px relative rounded-[2.13569e+07px] shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1),0px_4px_6px_-4px_rgba(0,0,0,0.1)] shrink-0 cursor-pointer"
      data-name="Button"
      onClick={onClick}
      role="button"
      tabIndex={0}
...
function Container20({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  return (
    <div className="content-stretch flex gap-[11.994px] h-[47.985px] items-center relative shrink-0 w-full" data-name="Container">
      <Button4 onClick={() => setActiveTab?.('home')} />
      <Button5 onClick={() => setActiveTab?.('sessions')} />
      <Button6 onClick={() => setActiveTab?.('assistant')} />
      <Button7 onClick={() => setActiveTab?.('profile')} />
    </div>
  );
}
...
function BottomNav({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  return (
    <div className="absolute z-50 bg-[#1a1a1a] content-stretch flex flex-col h-[71.972px] items-start left-[49px] pb-0 pt-[11.994px] px-[23.997px] rounded-[2.13569e+07px] shadow-[0px_25px_50px_-12px_rgba(0,0,0,0.25)] top-[1035px] w-[275.916px]" data-name="BottomNav">
      <Container20 setActiveTab={setActiveTab} />
    </div>
  );
}
