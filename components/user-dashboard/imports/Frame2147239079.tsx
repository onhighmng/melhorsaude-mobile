...
function Container6({ date, time }: { date: string; time: string }) {
  return (
...
function Container7({ onClose, session }: { onClose?: () => void; session?: any }) {
  return (
    <div className="absolute bg-[#0b74a5] h-[275.25px] left-0 rounded-[18px] shadow-[0px_18.75px_37.5px_-9px_rgba(0,0,0,0.25)] top-0 w-[279.75px]" data-name="Container">
      ...
      <Container6 date={session?.date || 'Hoje'} time={session?.time || '14:00'} />
...
function Frame({ onClose, session }: { onClose?: () => void; session?: any }) {
  ...
  const pillarTitle = getPillarLabel(session?.pillar);

  return (
    <div className="absolute h-[275.25px] left-0 top-0 w-[279.75px]">
      <Container7 onClose={onClose} session={{ ...session, pillar: pillarTitle }} />
    </div>
  );
}
...
function Button2() {
  return (
    <div className="absolute bg-[#e8f5e9] h-[42px] left-[93.46px] rounded-[79.883px] top-[6px] w-[87.073px] cursor-pointer hover:bg-[#c8e6c9] transition-colors flex items-center justify-center" data-name="Button">
...
function Menu({ onReschedule, onCancel }: { onReschedule?: () => void; onCancel?: () => void }) {
  return (
...
export default function Frame1({ onClose, onReschedule, onSessionCancel, session }: { onClose?: () => void; onReschedule?: () => void; onSessionCancel?: () => void; session?: any }) {
  ...
  const handleCancelConfirm = () => {
    ...
    // onClose will be handled by parent after successful cancel or immediately if desired
    if (onClose) {
      onClose();
    }
  };
    ...
    <>
      ...
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 pointer-events-none">
        ...
        >
          ...
          <Menu onReschedule={onReschedule} onCancel={handleCancelClick} />
        </div>
      </div>
