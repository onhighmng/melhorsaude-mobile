...
function SessionRow({ session, onClick }: { session: SessionInfo; onClick: () => void }) {
  ...
  const getPillarLabel = (pillar: string) => {
    ...
    if (p === 'FINANCIAL' || p === 'FINANCEIRA' || p === 'FINANCEIRO') return t('bemEstar.financial');
    if (p === 'PSYCH' || p === 'MENTAL' || p === 'SAUDE_MENTAL') return t('bemEstar.mental');
    if (p === 'PHYSICAL' || p === 'FÍSICO' || p === 'FISICO' || p === 'BEM_ESTAR_FISICO' || p === 'BEM-ESTAR FÍSICO') return t('bemEstar.physical');
    if (p === 'LEGAL' || p === 'JURÍDICA' || p === 'JURIDICA' || p === 'LEGAL_SOCIAL' || p === 'ASSISTENCIA_JURIDICA') return t('bemEstar.legal');
    return pillar;
  };
    ...
    >
      <div className="flex flex-row items-center size-full px-3">
        ...
        <div className="flex flex-col items-end justify-center shrink-0 pr-1">
          {session.date && (
            <p className="font-['Inter:Regular',sans-serif] font-medium text-[10px] text-gray-500 leading-tight">
              {session.date}
            </p>
          )}
          <p className="font-['Inter:Regular',sans-serif] font-semibold text-[13px] text-[#101828] leading-tight">
            {session.time}
          </p>
        </div>
      </div>
...
export default function Frame2147239064({ onReschedule, sessions = [] }: { onReschedule?: (sessionId: string, pillar: string) => void, sessions?: SessionInfo[] }) {
  const { t } = useLanguage();
  ...
  return (
    <div className="relative size-full">
      <div className="absolute h-[345px] left-0 rounded-[32px] top-0 w-full overflow-hidden">
        ...
        {/* Button */}
        {/* Button - defaulted to first session if available for interaction, or just generic */}
      </div>
      ...
      {showFrame2147239079 && selectedSession && (
        <Frame2147239079
          session={selectedSession}
          onClose={() => setShowFrame2147239079(false)}
          onReschedule={() => {
            setShowFrame2147239079(false);
            if (onReschedule && selectedSession) {
              onReschedule(selectedSession.id, selectedSession.pillar);
            } else {
              console.warn('Parent onReschedule is missing');
            }
          }
          }
          onCancel={async () => {
