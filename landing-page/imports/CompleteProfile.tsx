    ...
    >
      <div className="flex flex-row items-center justify-center px-[20px] py-[14px]">
        <div className="flex flex-col font-['SF_Pro_Text:Semibold',sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-[17px] text-center text-nowrap text-black tracking-[-0.408px]">
          <p className="leading-[22px]">Tudo pronto, continuar</p>
        </div>
      </div>
    </div>
  );
...
function Field({ label, children }: { label: string, children: React.ReactNode }) {
  return (
    <div className="w-full flex flex-col gap-2">
      <label className="font-['SF_Pro_Text:Semibold',sans-serif] text-[16px] text-white font-bold ml-4">{label}</label>
      <Wrapper>
        {children}
      </Wrapper>
    </div>
    ...
    <div className="flex flex-col gap-6 w-full max-w-[345px]">
      ...
      <Field label="Full Name">
        <input
          ...
          className="w-full bg-transparent border-none outline-none font-['SF_Pro_Text:Regular',sans-serif] text-[15px] text-black placeholder-black/50"
        />
      </Field>
      ...
      <Field label="Select Your Job">
        <select className="w-full bg-transparent border-none outline-none font-['SF_Pro_Text:Regular',sans-serif] text-[15px] text-black appearance-none cursor-pointer">
